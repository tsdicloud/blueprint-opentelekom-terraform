...
data "external" "topo" {
  ...
  # strings
  program = ["jq", "[.[]|select(.tf_type==\"terra\")|select( .contents[0]==\"${var.main_service}\")|.contents|=join(\",\")|.[]|=if type==\"number\" then tostring else . end]+[{}]|.[0]", "${var.topo_file}"]
}
...
locals {
  ...
  terra_user             = "${lookup(data.external.topo.result, "terra_user", var.terra_user)}"
  iam_instance_profile = "${lookup(data.external.topo.result, "iam_instance_profile", var.iam_instance_profile)}"
  ...
  mount_efs            = "${lookup(data.external.topo.result, "mount_efs", "16")}"
}

module "create-topo-terra" {
  source = "../terra"
